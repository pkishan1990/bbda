				...
				function(root, daterangepicker, moment, $) {
					...
					DateRangePicker.prototype = {
						...
						setOptions : function(options, callback) {
							...
							if (this.opens == 'right' || this.opens == 'center') {
								...
								var first = this.container
										.find('.calendar.first');
								var second = this.container
										.find('.calendar.second');
						...
						updateFromControl : function() {
							...
							if (dateString.length === 2) {
								start = moment(dateString[0], this.format)
										.zone(this.timeZone);
								end = moment(dateString[1], this.format).zone(
										this.timeZone);
							}
						...
						updateInputText : function() {
							if (this.element.is('input')
									&& !this.singleDatePicker) {
								var from = '#' + this.locale.fromLabel;
								var to = '#' + this.locale.toLabel;
								if (from != to) {
									$(from).val(
											this.startDate.format(this.format));
									$(to).val(this.endDate.format(this.format));
								} else {
						...
						updateTime : function(e) {
							...
							if (this.timePickerSeconds) {
								second = parseInt(cal.find('.secondselect')
										.val(), 10);
							}
									...
									.html(
											this
													.renderCalendar(
															...
															this.endDate,
															this.singleDatePicker ? this.minDate
																	: this.startDate,
															this.maxDate,
															'right'));
						...
						renderCalendar : function(calendar, selected, minDate,
							...
							for ( var row = 0; row < 6; row++) {
								...
								for ( var col = 0; col < 7; col++) {
									...
									cname += (calendar[row][col].month() == calendar[1][1]
											.month()) ? '' : 'off';
									var date = calendar[row][col].day();
									var a = $('#status').val().split("[")[1]
											.split("]")[0].split(",");
									for ( var i = 0; i < a.length; i++) {
										if (calendar[row][col]
												.format('YYYY-MM-DD') == a[i]
												.trim()) {
											cname = ' off disabled ';
										}
									}
							...
							if (this.timePicker) {
								...
								if (minDate
										...
										&& selected.format('YYYY-MM-DD') == minDate
												.format('YYYY-MM-DD')) {
								...
								if (maxDate
										...
										&& selected.format('YYYY-MM-DD') == maxDate
												.format('YYYY-MM-DD')) {
								...
								if (minDate
										&& (side == 'left' || this.singleDatePicker)
										&& selected.format('YYYY-MM-DD h A') == minDate
								...
								if (maxDate
										&& (side == 'right' || this.singleDatePicker)
										&& selected.format('YYYY-MM-DD h A') == maxDate
												.format('YYYY-MM-DD h A')) {
