...
;(function($, window, undefined)
{
	...
	$(document).ready(function()
	{
		setInterval(function(){ 
			var semiBoldClass = $('#errorMsg').hasClass("text-normal");
			if(semiBoldClass){
				$('#errorMsg').removeClass("text-normal");
				$('#errorMsg').removeClass("bold");
			}else{
				$('#errorMsg').addClass("text-normal");
				$('#errorMsg').addClass("bold");
			}
		}, 1000);
		
		
		var sessioncheck = $('#errorMsg').text();
		if(sessioncheck == "Session Expired !!!"){
				...
				setTimeout(function()
				{
					...
					$.ajax({
						...
						error: function()
						{
							...
							$('#errorMsg').css('letter-spacing','10%');
							 ...
							 var formOptions = "";
					    	   $(xml).find("dropdownlist").each(function()
					    		   ...
					    		   var arrayData = formOptions.split("/");
					    		   		...
					    		   		login_status = arrayData[0]; 
						    		    error_msg = arrayData[1];
						    		    baseurl =  arrayData[2];
					...
					*/
					...
					$.ajax({
						...
						beforeSend: function(){
							setTimeout(function()
									{
										var random_pct = 25 + Math.round(Math.random() * 10);
						    		    	...
						    		    	setTimeout(function()
													{
														...
														neonLogin.setPercentage(random_pct, function()
														{
															setTimeout(function()
																	{
																		var random_pct = 15 - Math.round(Math.random() * 10);
																		neonLogin.setPercentage(random_pct, function()
																		{
