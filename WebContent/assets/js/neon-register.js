...
;(function($, window, undefined)
{
	...
	$(document).ready(function()
	{
		$('#empcode').blur(function(){
			...
			var  dataString = "empcode="+empCode;
//			var  dataString = $("#form_register").serialize();
			
			$.ajax({
				type: "POST",
				 url: "populateEmployeeData.htm",
				 data: dataString,
				 dataType: "json",
				 error: function(data, textStatus, jqXHR){ 
			    	   alert("Error retriving employe details..");
			      },
			       beforeSend:function(){
			       ...
			       success: function(response) {
			    		   ...
			    		   $('#username').val(response.Name.fullname);
			    		   $('#userlevel').val(response.Name.role);
			    		   $('#rolename').val(response.role);
			    		   $('#phone').val(response.Name.phone1);
			    		   $('#email').val(response.Name.emailwork);
				...
				neonRegister.setPercentage(30, function()
				{
					...
					neonRegister.setPercentage(98, function()
					{
						...
						$.ajax({
							...
							error: function()
							{
								alert("An error occoured while registering user!");
							},
							success: function()
							{
